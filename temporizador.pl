...
if ( $conf_file && open $CONF, "<", $conf_file ) {
    while ( my $linha = <$CONF> ) {
        $linha =~ /^\s*(\w+)\s*:\s*(.*)\s*$/;
        $conf{$1} = $2;
    }
...
unless ( exists $conf{dbname} ) {
    for my $arq (
        $ENV{HOME} . "/temporizador.sql",
        $conf{root} . "/temporizador.sql",
        qw{temporizador.sql /var/temporizador.sql}
      )
    {
        if ( -f $arq ) {
            $conf{dbname} = $arq;
...
our $temp = temporizador->new(
    "dbi:$conf{banco}:dbname=$conf{dbname}"
      . (
        $conf{dbhost}
        ? ";host=$conf{dbhost}"
        : ""
...
my $on =
  Gtk2::Image->new_from_pixbuf(
    Gtk2::Gdk::Pixbuf->new_from_file("$base/$conf{icone_on}") );
my $off =
  Gtk2::Image->new_from_pixbuf(
    Gtk2::Gdk::Pixbuf->new_from_file("$base/$conf{icone_off}") );
...
sub muda_tooltip {
    ...
    $event->set_tooltip_text( ' ' . $nome_projeto . ' (' . $tempo . ') ' );
...
sub menu {
    ...
    for my $proj ( $temp->get_projetos ) {
        ...
        $menu_proj->signal_connect(
            activate => sub { muda_projeto( $eventbox, $proj->id ) } );
        $menu->add($menu_proj);
    }
...
sub loginout {
    ...
    if ( $temp->is_logged_in ) {
        ...
        $timer = Glib::Timeout->add( 1000 * 60 * $conf{tempo_alerta}, \&timer )
          if $conf{tempo_alerta};
    }
...
sub subrotina {
    ...
    if ( -d $File::Find::name ) {
        ...
        push @criados, $File::Find::name
          if defined $mod_cri && $mod_cri eq "criado";
        push @modificados, $File::Find::name
          if defined $mod_cri && $mod_cri eq "modificado";
    }
}
